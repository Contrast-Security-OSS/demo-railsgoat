                ...
                script {
                    ...
                    withCredentials([azureServicePrincipal('ContrastAzureSponsored')]) {
                        try {
                            ...
                            terraform apply -auto-approve -var 'location=$location' -var 'initials=$initials' -var 'environment=development' -var 'servername=Macbook-Pro' -var 'session_metadata="branchName=${env.GIT_BRANCH},buildNumber=${BUILD_NUMBER},commitHash=${env.GIT_SHORT_COMMIT},version=1.0"' -var 'run_automated_tests=true'
                            """
                        } catch (Exception e) {
                            echo "Terraform refresh failed, deleting state"
                            sh "rm -rf terraform.tfstate"
